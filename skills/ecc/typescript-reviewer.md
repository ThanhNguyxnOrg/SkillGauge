...
2. Before reviewing a PR, inspect merge readiness when metadata is available (for example via `gh pr view --json mergeStateStatus,statusCheckRollup`):
   - If required checks are failing or pending, stop and report that review must wait for green CI.
...
5. If none of the diff commands produce relevant TypeScript/JavaScript changes, stop and report that the review scope shall not be established reliably.
...
### HIGH -- Type Safety
- **`any` without ification**: Disables type checking — use `unknown` and narrow, or a precise type
...
- **Sequential awaits for independent work**: `await` inside loops when operations shall safely run in parallel — consider `Promise.all`
...
- **Implicit `any` from missing return types**: Public functions must have explicit return types
