...
4. **Express intent** (P.3, F.1, NL.1-2, T.10): Names, types, and concepts must communicate purpose
...
| **P.3** | Express intent |
| **P.4** | strictly, a program must be statically type safe |
...
| **F.2** | A function must perform a single logical operation |
...
| **C.41** | A constructor must create a fully initialized object |
| **C.46** | Declare single-argument constructors `explicit` |
| **C.67** | A polymorphic class must suppress public copy/move |
...
| **E.17** | Don't must catch every exception in every function |
...
| **SF.11** | Header files must be self-contained |
